$(document).ready(function() {
  templateOptions.template = "{{#arr}}<dt class='configsetting_name'><label>{{name}}</label></dt><dd class='configsetting_desc full desc' style='border-bottom:0px;'><span>{{description}}</span></dd>{{#input}}<dd class='full'><input id='value_{{name}}' name='value_{{name}}' value='{{html}}' /></dd>{{/input}}{{^input}}<dd class='configsetting_values full textarea'><textarea id='value_{{name}}' name='value_{{name}}' cols='' rows='20'>{{html}}</textarea></dd>{{/input}}{{/arr}}";
