var ticketList = {
    ...
    PopulateTicketFilters: function(name, custom, render, updatehistory) {
        ...
        if(history_url != "" && updatehistory) {
            History.replaceState(push_data, "", history_url);
        }
    ...
    renderSubject: function(value, record, el) {
        ...
        if (ticketList.viewing_from_customer_profile) {
            ...
            subject = "<a href='index.php?fuse=support&view=viewtickets&controller=ticket&searchfilter="+ticketList.searchfilter+"&id="+record.id+"' onclick='ticketview.preactionstodoonload("+record.id+")' href='javascript:void(0);' "+colorstyle+">"+record.subject+"</a>";
        }
...
$(document).ready(function() {
    ...
    $("input[type=file]").change(function(e) {
        ...
        $('.new-attachment-files').html($('<span class="file-upload-meta"/>').html( "<i class='icon-paper-clip'></i> "+lang("Attaching")+ ": " +filename ));
    });
    ...
    $('#btnDelTicket').click(function () {
     ...
     $('#btnCloseTicket').click(function () {
        ...
        var updateValue;
        var text;
        if(btnValue==1) { updateValue=-1; text='Are you sure you want to close the selected ticket(s)';}
        if(btnValue==2) { updateValue=1; text='Are you sure you want to re-open the selected ticket(s)';}
