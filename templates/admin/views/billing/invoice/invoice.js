...
$(document).ready(function(){
    ...
    invoiceview.calculatetotals = function() {
        ...
        $('input.invoiceentry-price').each(function(type,el){
            ...
            if(entrytypeid == -3 && entrytaxed !== "0"){
                ...
                entrytax1formatted = invoiceview.calculatetaxforprice(price,entrytaxed,1);
                entrytax1formatted = entrytax1formatted.toString();
                invoiceview.tax1total += accounting.unformat(entrytax1formatted.replace(invoiceview.currency.symbol, ""), invoiceview.currency.decimalssep);
                entrytax2formatted = invoiceview.calculatetaxforprice(price,entrytaxed,2);
                entrytax2formatted = entrytax2formatted.toString();
                invoiceview.tax2total += accounting.unformat(entrytax2formatted.replace(invoiceview.currency.symbol, ""), invoiceview.currency.decimalssep);
            }
        });
    ...
    invoiceview.clickanchorsafterchanges = function(e)
    {
        ...
        if (href === "") {
            //console.debug('Error: there was not an href attribute on the node you just clicked.');
        }
    ...
    invoiceview.calculatetaxforprice = function(price,entrytaxed,level) {
        var tax;
    ...
    invoiceview.bindentries = function(){
        ...
        $('.icon_menu').bind('hover', function() {
            $(this).hide();}, function () { /*console.debug('out')*/;$(this).show();} );
        ...
        $('ul.dropdown-menu:not(".invoice-entry-actions") li span').click(function(){
            var text = $(this).text();
        ...
        $('tr span.edit-entry').click(function(){
            ...
            var content = '<label>Description</label><input type="text" style="width:305px;" name="description" placeholder="Description" value="'+$(descId).text()+'" /><br/><br/>';
            ...
            if (startId.first().text() != '') {
              ...
              content += "<a href='#'><i class='icon-remove-sign icon-large'></i></a>";
              content += "<legend>"+lang("Period dates")+"</legend>";
            ...
            $('.editInvoiceEntryPopup > a').click(function() {
