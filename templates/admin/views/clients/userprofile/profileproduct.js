...
packagemanager.returnFieldHTML = function(data)
{
    ...
    //let's make sure we don't count warning to length count
    //we remove two one for the warning hidden field as well as for the hidden label
    length = json.productFields.length;
    if ($('#hiddenwarning').length > 0) length = length - 2;
    if ($('#updateviaplugin').length > 0) length = length - 1;
    ...
    //console.debug(packagemanager.permissions);
...
*/
packagemanager.showManageSettingByType = function(typeid,updatePlugins) {
    ...
    if (updatePlugins) packagemanager.updateAvailablePluginActions();
...
$(document).ready(function(){
    ...
    $(".vtab").removeClass('active');
    if ($('#selectedproduct-view').length > 0) {
        packagemanager.showManageSettingByType(packagemanager.activeTab,true);
