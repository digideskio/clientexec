...
clientexec.addNewEvent = function(text,header,gravataremail,date){
        ...
        if (gravataremail && gravataremail!="") {
            gravataremail = ce.getGravatarUrl(gravataremail);
        } else {
            gravataremail = "../templates/admin/images/logo_in1.png";
        }

        event = {"text":text,"header":header,"url":gravataremail, "time" : date};
        event.calctime = function () {
            ...
            // console.debug("serverts"+this.time+" | server:"+time2,"diff(withoutoffset):"+Math.ceil(((time1 - time2) / 1000)/60),"offset"+tzDifference,"diff(withoffset)"+Math.ceil( (((time1 - time2) / 1000)/60) + tzDifference));
...
clientexec.popupKBArticle = function(id,title)
{
    ...
    clientexec.kbarticle = new RichHTML.window({
        ...
        actionUrl: 'index.php?controller=cannedresponse&action=savecannedresponse&fuse=support',
        title: title
    });
...
clientexec.postpageload = function(parent)
{
    ...
    $(parent+' textarea.wysihtml5:not([data-withembed])').redactor({parentSel: '#product-tab-content',autoresize: false,plugins: ['clips'],
    ...
    $(parent+' textarea.wysihtml5[data-withembed]').redactor({parentSel: '#product-tab-content',
        ...
        autoresize: false,plugins: ['clips'],
...
clientexec.updateticketfiltercounts = function(response) {
    ...
    if (waiting_tickets_count == 0) {
        $('.nav_support .topmenu-count-label').removeClass('label-important').addClass('label-inverse');
...
clientexec.updateonlineusers = function(response)
{
    ...
    //console.debug(clientexec.whoisonline.userCount);
...
clientexec.updatevitals = function(needyourattention) {
    ...
    if (needyourattention.length > 0) {
         $.each(needyourattention,function(i,obj) {
            ...
            if (obj['menu-class'] == "menu-pending-orders") {
                pending_users_num = obj['count'];
            } else if (obj['menu-class'] == "menu-invoices-ready" && clientexec.settings.billing.showReadyInvoiceCount ) {
                ouststanding_invoices_count = parseInt(ouststanding_invoices_count) + parseInt(obj['count']);
    ...
    //pending users menu count
    if (pending_users_num == 0) {
        $('.nav_accounts .topmenu-count-label').removeClass('label-important').addClass('label-inverse');
    } else if ($('.nav_accounts .topmenu-count-label').length == 0) {
        new_el = $('<span>').addClass('topmenu-count-label label label-important').text(pending_users_num);
        $(new_el).insertBefore('.nav_accounts .drop');
    } else {
        $('.nav_accounts .topmenu-count-label').removeClass('label-inverse').addClass('label-important').text(pending_users_num);
    }
...
ce.addSupportTicket = function(a1,a2) {
    ...
    supportticket.window = new RichHTML.window({
        ...
        actionUrl: 'index.php?fuse=support&action=saveticket&controller=ticket',
        showSubmit: true,
        title: lang("Add Support Ticket"),
        onSubmit: function(response) {

            if ( response.success === true ) {
                window.location = 'index.php?fuse=support&id=' + response.ticketId + '&view=viewtickets&controller=ticket';
            }
        }
    });
...
function togglePackageListType(type) {
    if (type == 'domain') {
        window.location = 'index.php?fuse=clients&controller=packages&view=domainslist';
    } else if (type == 'hosting') {
        window.location = 'index.php?fuse=clients&controller=packages&view=hostingpackagelist';
    } else if (type =='general') {
        window.location = 'index.php?fuse=clients&controller=packages&view=generalpackageslist';
    } else if (type == 'ssl') {
        window.location = 'index.php?fuse=clients&controller=packages&view=sslpackagelist';
    }
}
