...
require_once 'library/CE/3rdparty/Gravatar/TalkPHP_Gravatar.php';
...
class SSEData {
	...
	public function __construct($db){
        $this->db = $db;
        $this->pAvatar = new TalkPHP_Gravatar();
	}
	...
	public function save_files($roomid, $file)
	{
		...
		//file_put_contents("test.txt",serialize(print_r(array(mysqli_insert_id(), $roomid, $file->original_name,$file->name,$file->size),true)),FILE_APPEND);
	...
	public function getMessages($roomid,$id,$chatterid)
	{
        ...
        // specify MYSQL_ASSOC to avoid revealing other information in indexed numbers
		while ($row = $result->fetch(MYSQL_ASSOC)) {
            $this->pAvatar->setEmail($row['email'])->setSize(80)->setRatingAsPG()->setDefaultImageAsWavatar();
            $row['gravatar'] = html_entity_decode($this->pAvatar->getAvatar());
	...
	public function get_all_active_admin_users()
	{
		...
		while ($row = $result->fetch()) {
			...
			while ($row2 = $resultfornickname->fetch()) {
				if (trim($row2['nickname']) == "") continue;
