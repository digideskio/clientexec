...
class Overdue_Invoices extends Report {
    ...
    public function process() {
        ...
        $aGroup = array();
        $aLabel = array("CE PKG Status","Invoice Date","Invoice #","Amount","CE ID","CE PKG");
            ...
            if(count($OverdueInvoiceData["CE PKG"]) > 0){
                foreach($OverdueInvoiceData["CE PKG"] as $OverdueInvoice_CE_PKG){
                    ...
                    $group = array(
                        ...
                        "<a href='index.php?fuse=clients&controller=userprofile&view=profilecontact&frmClientID=".$OverdueInvoiceData["CE ID"]."'>".$OverdueInvoiceData["CE ID"]."</a>",
                        $CE_PKG
                    );
                    $aGroup[] = $group;
                }
        ...
        echo '<tr>';
        echo '<td>';
        echo "<button class='btn".$disablePrevious."' type='button' data-loading-text='Loading...' onclick='window.location.href=\"index.php?fuse=reports&view=viewreport&controller=index&report=Overdue+Invoices&type=Income&page=".($page-1)."&results=".$count."\"'".$disablePrevious."><&nbsp;&nbsp;"
            .$this->user->lang("Previous") . "</button>";
        echo "&nbsp;&nbsp;&nbsp;";
        echo "<button class='btn".$disableNext."' type='button' data-loading-text='Loading...' onclick='window.location.href=\"index.php?fuse=reports&view=viewreport&controller=index&report=Overdue+Invoices&type=Income&page=".($page+1)."&results=".$count."\"'".$disableNext.">"
    ...
    {
        $csv = '"CE PKG Status","Invoice Date","Invoice #","Amount","CE ID","CE PKG"'."\n";
