        ...
        while($row = $result->fetch()){
            ...
            while($row2 = $result2->fetch()){
                ...
                if($row2['type'] == 3){  // Domain Type
                    ...
                    if($row['paymentterm'] != 0 && ($row['paymentterm'] % 12) == 0 && isset($pricing['pricedata'][$tld][($row['paymentterm']/12)]['price'])){
                        $packagePrice = $pricing['pricedata'][$tld][($row['paymentterm']/12)]['price'] / $row['paymentterm'];
                ...
                while ($row3 = $result3->fetch()) {
                    if($row3['paymentterm']== 3){
                            $row3['total'] = $row3['total'] / 3;
                            $append = "Quarterly";
                    }elseif($row3['paymentterm']== 6){
                            $append = "Semi-Annualy";
                            $row3['total'] = $row3['total'] / 6;
                    }elseif($row3['paymentterm']== 12){
                            $append = "Annually";
                            $row3['total'] = $row3['total'] / 12;
                    }elseif($row3['paymentterm']== 24){
                            $append = "Bi-Annually";
                            $row3['total'] = $row3['total'] / 24;
                    }elseif($row3['paymentterm']== 1){
                            $append = 'Monthly';
                    } else {
                        continue;
                    }
