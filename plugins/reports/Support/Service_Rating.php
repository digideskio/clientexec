...
class Service_Rating extends Report {
    ...
    function process() {
        ...
        if ($graphdata) {
            ...
            $sql = "select MONTH(lastlog_datetime) as month, DATE_FORMAT(lastlog_datetime,'%Y-%m-1') as rate_date, rate from troubleticket WHERE (YEAR(lastlog_datetime) = " . $year . ") AND status IN (".implode(', ', $statusClosed).") AND rate > 0 ORDER BY lastlog_datetime ASC";
