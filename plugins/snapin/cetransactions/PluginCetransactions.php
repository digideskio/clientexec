...
class PluginCetransactions extends SnapinPlugin
{
    ...
    private function getTransactions($sort, $dir, $start, $limit)
    {
        ...
        if ($invoiceid != 0) {
            ...
            $query = "SELECT it.id, it.accepted, it.response, UNIX_TIMESTAMP(it.transactiondate) AS transactiondate, it.invoiceid, it.transactionid, it.action, it.last4, it.amount, i.pluginused, i.customerid FROM invoicetransaction it, invoice i WHERE i.id = it.invoiceid AND invoiceid in (SELECT id FROM `invoice` WHERE `customerid` = ".$userid.")";
        } else {
