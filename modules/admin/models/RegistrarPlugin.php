...
abstract class RegistrarPlugin extends NE_Plugin
{
     ...
     *
     * @param array $params Contains the values for the variables defined in getVariables(), plus: nsname.
     *
     * @abstract
     * @return string Status string
     */
    abstract function checkNSStatus($params);

    /**
     * Communicates with the registrar API to register a name server.
     *
     * @param array $params Contains the values for the variables defined in getVariables(), plus: nsname, nsip.
     *
     * @abstract
     * @return string Status string
     */
    abstract function registerNS($params);

    /**
     * Communicates with the registrar API to edit a name server.
     *
     * @param array $params Contains the values for the variables defined in getVariables(), plus: nsname, nsnewip, nsoldip.
     *
     * @abstract
     * @return string Status string
     */
    abstract function editNS($params);
     ...
     * @return string Status string
     */
    abstract function deleteNS($params);
    ...
    function buildRenewParams($userPackage, $params = false)
    {
        ...
        if (($position = strpos($domainName, '.')) === false) {
            list($sld, $tld) = array(
                $domainName,
                ''
            );
            ...
            list($sld, $tld) = array(
                mb_substr($domainName, 0, $position),
                mb_substr($domainName, $position + 1)
            );
        }
        ...
        $params['NumYears'] = $dng->getPeriod($userPackage);
    ...
    function buildRegisterParams($userPackage, $params = false)
    {
        ...
        if ( $result->getNumRows() == 0 ) {
            ...
            $result = $this->db->query($query, $userPackage->getCustomField('Server Id'));
