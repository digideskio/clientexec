...
class Admin_menu extends NE_MenuHook
{
    ...
    function Admin_menu($user)
    {
        ...
        $submenuItem = new NE_MenuItem($user->lang("Style"),"index.php?fuse=admin&controller=settings&view=templateoptions&settings=style_options");
        $submenu->addItem($submenuItem);
        ...
        if ( $user->isSuperAdmin() || $user->hasPermission('admin_manage_customer_group')  ) {
            ...
            if ( $user->hasPermission('admin_manage_customer_group') ) {
                $submenuItem = new NE_MenuItem($user->lang("Customer Groups"),"index.php?fuse=admin&controller=groups&view=viewgroups");
                $submenu->addItem($submenuItem);
            }
        ...
        if ( $user->hasPermission('admin_manage_plugins') ) {
            ...
            $submenuItem = new NE_MenuItem($user->lang("Payment Processors"),"index.php?fuse=admin&controller=settings&view=plugins&settings=plugins_gateways&type=ThirdPartyBilling");
            $submenu->addItem($submenuItem);
